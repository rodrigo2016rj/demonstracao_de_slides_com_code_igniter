...
use App\Models\SlideReactModel;

final class SlideReactController extends TemplateController{

  public function index(){
    $this->mostrar_itens();

    $this->get_smarty()->display('slide_react/slide_react.html');
    die;
  }

  private function mostrar_itens(){
    $slide_react_model = new SlideReactModel();

    /* Preparando o resultado */
    $itens = $slide_react_model->selecionar_ultimos_oito_itens();
    $array_itens = array();
    $posicao_no_slide = 0;
    foreach($itens as $item){
      ...
      $subitens = $slide_react_model->selecionar_ultimos_seis_subitens_do_item($id_do_item);
      $posicao_na_lista_de_subitens = 0;
      foreach($subitens as $subitem){
        $id_do_subitem = $subitem->get_pk_subitem();
        $array_subitem['id'] = $id_do_subitem;

        $posicao_na_lista_de_subitens++;
        $array_subitem['posicao_na_lista'] = $posicao_na_lista_de_subitens;

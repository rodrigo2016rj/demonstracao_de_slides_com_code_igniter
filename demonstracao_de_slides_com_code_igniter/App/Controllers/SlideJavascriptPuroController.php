...
use App\Models\SlideJavascriptPuroModel;

final class SlideJavascriptPuroController extends TemplateController{

  public function index(){
    $this->mostrar_itens();

    $this->get_smarty()->display('slide_javascript_puro/slide_javascript_puro.html');
    die;
  }

  private function mostrar_itens(){
    $slide_javascript_puro_model = new SlideJavascriptPuroModel();

    /* Preparando o resultado */
    $itens = $slide_javascript_puro_model->selecionar_ultimos_oito_itens();
    $array_itens = array();
    $posicao_no_slide = 0;
    foreach($itens as $item){
      ...
      $subitens = $slide_javascript_puro_model->selecionar_ultimos_seis_subitens_do_item($id_do_item);
